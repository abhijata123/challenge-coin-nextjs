/*
  # Add notes column to pending_coin_transfers table

  1. Schema Changes
    - Add `notes` column to `pending_coin_transfers` table to store the note when sending coins to new users

  2. Function Updates
    - Update `transfer_coins_with_note` function to store the note in pending transfers
    - Update `process_pending_transfers` function to use the stored note when creating coins for new users

  3. Security
    - Maintain existing RLS policies
    - Ensure proper email normalization for consistency
*/
...
-- Update the transfer_coins_with_note function to store notes in pending transfers
...
BEGIN
    ...
    IF receiver_exists THEN
        ...
        RETURN 'success';
    ELSE
        -- Receiver doesn't exist, create pending transfer with notes
        INSERT INTO pending_coin_transfers (
            ...
            backside_url,
            quantity,
            notes -- Store the note for later use
        ) VALUES (
            ...
            original_backside_url,
            p_quantity,
            COALESCE(p_note, '') -- Store the provided note or empty string
        );
...
-- Update the process_pending_transfers function to use stored notes
    ...
    LOOP
        ...
        FOR i IN 1..pending_transfer.quantity LOOP
            INSERT INTO "Challenge Coin Table" (
                ...
                NEW."Username",
                COALESCE(pending_transfer.notes, 'This coin was sent to you before you joined!'), -- Use stored note or default message
                null,
                null,
                false, -- New coins are not featured by default
                false, -- New coins are private by default
