/*
  ...
  2. Changes
    ...
    - Add process_pending_transfers function for new user registration
...
-- Create pending_coin_transfers table if not exists
...
-- Function to process pending transfers when a new user registers
...
BEGIN
  ...
  -- Check if receiver exists and handle accordingly
  ...
  -- Handle transfer for registered user
  IF EXISTS (
    SELECT 1 
    FROM "Challenge Coin Table"
