/*
  # Add question and answer functionality for coins
  
  1. New Tables
    - coin_questions: Store questions and answers about coins
    
  2. Changes
    - Add notification types for questions and answers
...
-- Create coin_questions table if it doesn't exist
...
DROP FUNCTION IF EXISTS notify_question_asked() CASCADE;
DROP FUNCTION IF EXISTS notify_answer_added() CASCADE;
...
BEGIN
  ...
  -- Log for debugging
  RAISE NOTICE 'Creating question notification: Coin ID: %, Owner: %, Questioner: %, Question: %', 
    NEW.coin_id, v_coin_owner, v_questioner_username, NEW.question;
...
BEGIN
  ...
  IF OLD.answer IS NULL AND NEW.answer IS NOT NULL THEN
    ...
    -- Log for debugging
    RAISE NOTICE 'Creating answer notification: Question ID: %, Coin ID: %, Questioner: %, Answerer: %, Answer: %', 
