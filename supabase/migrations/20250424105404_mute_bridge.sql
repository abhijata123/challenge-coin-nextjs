-- Drop rarity level column if it exists
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 
...
-- Drop rarity level from pending_coin_transfers if it exists
DO $$ 
BEGIN
  IF EXISTS (
    SELECT 1 
...
-- Update transfer_coins function without rarity level
...
-- Update process_pending_transfers function without rarity level
