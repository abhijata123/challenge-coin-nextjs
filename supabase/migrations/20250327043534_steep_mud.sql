/*
  # Fix coin transfer constraints and cleanup

  1. Changes
    - Drop existing foreign key constraints
    - Recreate coin_transfers table with proper constraints
    - Update transfer function to handle constraints
    - Add cascade delete for transfers when coins are deleted
...
-- Drop existing coin_transfers table and recreate with proper constraints
...
-- Function to transfer coins with proper constraint handling
...
BEGIN
  ...
  -- Update transfer status and new coin reference
  UPDATE coin_transfers
  SET 
    status = 'completed',
    coin_id = v_new_coin_id
